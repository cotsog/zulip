...
def export_realm_data(realm, response):
    ...
    for (table, model) in realm_tables:
        response[table] = [model_to_dict(x) for x in
                           model.objects.filter(realm_id=realm.id)]
...
def export_with_admin_auth(realm, response, include_invite_only=True, include_private=True):
    ...
    user_recipients = [model_to_dict(x)
                       for x in Recipient.objects.filter(type=1)
    ...
    stream_recipients = [model_to_dict(x)
                         for x in Recipient.objects.filter(type=2, type_id__in=stream_ids)]
    ...
    if include_private:
        ...
        realm_huddle_subs = Subscription.objects.select_related("recipient").filter(recipient__type=3,
...
def fetch_usermessages(realm, message_ids, user_profile_ids, message_filename):
    ...
    #
    # TODO: Optimize this query to do the same thing more efficiently
    user_message_query = UserMessage.objects.filter(user_profile__realm=realm,
                                                    message_id__in=message_ids)
    user_message_chunk = [model_to_dict(um) for um in user_message_query if
                          um.user_profile_id in user_profile_ids]
...
def export_usermessages_batch(input_path, output_path):
    ...
    user_profile_ids = output['zerver_userprofile_ids']
...
def export_messages(realm, user_profile_ids, recipient_ids,
    ...
    while True:
        ...
        message_filename = os.path.join(output_dir, "messages-%04d.json" % (dump_file_id,))
...
def export_bucket(realm, bucket_name, output_dir, avatar_bucket=False):
    ...
    with open(os.path.join(output_dir, "records.json"), "w") as records_file:
        ujson.dump(records, records_file)
...
def export_messages_single_user(user_profile, chunk_size=1000, output_dir=None):
    ...
    while True:
        ...
        user_message_ids = set(um.id for m in user_message_chunk)
        ...
        for user_message in user_message_chunk:
            ...
            item['flags'] = um.flags_list()
            item['flags_mask'] = um.flags.mask
...
def fix_bitfield_keys(data, table, field_name):
    for item in data[table]:
        item[field_name] = item[field_name]['mask']
...
def do_import_realm(import_dir):
    ...
    if 'zerver_huddle' in data:
        # TODO: Make this conditional more coherent
    ...
    dump_file_id = 1
    while True:
        message_filename = os.path.join(import_dir, "messages-%04d.json" % (dump_file_id,))
